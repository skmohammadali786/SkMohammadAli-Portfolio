...
export default function Home() {
  return (
    <div className="relative overflow-hidden text-ink">
      <section className="mx-auto grid min-h-screen max-w-[1500px] items-center gap-12 px-4 pb-20 pt-32 sm:px-8 lg:grid-cols-[1.05fr_.95fr]">
        <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
          ...
          <h1 className="max-w-5xl text-[clamp(4.8rem,13vw,13.5rem)] font-black leading-[0.78] tracking-[-0.1em] text-ink">
            {portfolioData.name}
          </h1>
          <p className="mt-8 max-w-3xl text-xl font-semibold leading-9 text-ink/80 sm:text-2xl">{portfolioData.about.split("\n")[0]}</p>
          <div className="mt-10 flex flex-col gap-4 sm:flex-row">
            <a href="#projects" className="inline-flex items-center justify-center gap-2 rounded-full border-2 border-ink bg-fresh px-8 py-4 text-lg font-black text-ink shadow-[5px_5px_0_#2c2e2a] transition hover:-translate-y-1 hover:shadow-[8px_8px_0_#2c2e2a]">View Work <ArrowRight size={22} /></a>
            <a href="/resume.pdf" download className="inline-flex items-center justify-center gap-2 rounded-full border-2 border-ink bg-white px-8 py-4 text-lg font-black text-ink shadow-[5px_5px_0_#2c2e2a] transition hover:-translate-y-1 hover:shadow-[8px_8px_0_#2c2e2a]"><Download size={22} /> Download Resume</a>
          </div>
        </motion.div>
      ...
      <section id="about" className="mx-auto max-w-[1500px] px-4 py-24 sm:px-8"><SectionHeader eyebrow="About" title="A portfolio with one clear human thread." subtitle="The content remains SK Mohammad Ali’s portfolio; the visual language now follows MindMarket’s warm paper canvas, huge type, friendly illustrations, and bold connected-line motif." /><GlassPanel className="bg-white text-xl font-semibold leading-10 sm:text-2xl whitespace-pre-wrap">{portfolioData.about}</GlassPanel></section>

      <section id="skills" className="mx-auto max-w-[1500px] px-4 py-24 sm:px-8"><SectionHeader eyebrow="Capability Map" title="Skills, drawn like a living network." subtitle="Soft cards, thick ink outlines, and green structural accents create a closer match to the reference website’s illustrated system." /><div className="grid grid-cols-2 gap-5 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">{portfolioData.skills.map((skill, index) => <SkillBadge key={skill.name} name={skill.name} icon={iconMap[skill.icon]} delay={index * 0.035} />)}</div></section>

      <section id="experience" className="mx-auto max-w-[1500px] px-4 py-24 sm:px-8"><SectionHeader eyebrow="Journey" title="Professional signals, simplified." subtitle="Each role is presented as a confident editorial card with clear ownership and dates." /><div className="grid gap-5 lg:grid-cols-2">{portfolioData.experience.map((exp, index) => <GlassPanel key={exp.role} delay={index * 0.07} className="bg-white"><div className="flex h-full flex-col justify-between gap-10"><div><h3 className="text-4xl font-black tracking-[-0.07em]">{exp.role}</h3><p className="mt-2 text-xl font-black text-ink/70">{exp.company}</p><p className="mt-6 text-lg font-semibold leading-8 text-ink/78">{exp.description}</p></div><p className="inline-flex w-fit items-center gap-2 rounded-full border-2 border-ink bg-fresh px-4 py-2 font-black"><Calendar size={18} />{exp.period}</p></div></GlassPanel>)}</div></section>

      <section id="education" className="mx-auto max-w-[1500px] px-4 py-24 sm:px-8"><SectionHeader eyebrow="Foundation" title="Education that grounds the work." subtitle="Academic context retained exactly from the portfolio data." /><div className="grid gap-5 md:grid-cols-2">{portfolioData.education.map((edu, index) => <GlassPanel key={edu.degree} delay={index * 0.07} className={index % 2 ? "bg-[#f5e211]" : "bg-fresh"}><div className="flex items-start gap-5"><div className="rounded-[2rem] border-2 border-ink bg-white p-4 shadow-[4px_4px_0_#2c2e2a]"><GraduationCap size={30} /></div><div><h3 className="text-3xl font-black tracking-[-0.06em]">{edu.degree}</h3><p className="mt-3 text-xl font-semibold text-ink/78">{edu.institution}</p><p className="mt-6 inline-flex items-center gap-2 font-black"><Calendar size={18} />{edu.period}</p></div></div></GlassPanel>)}</div></section>

      <section id="projects" className="mx-auto max-w-[1500px] px-4 py-24 sm:px-8"><SectionHeader eyebrow="Featured Work" title="Projects as illustrated case cards." subtitle="Every project keeps its original title, description, link, and tags while adopting the warm illustrated reference style." /><div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{portfolioData.projects.map((project, index) => <ProjectCard key={project.title} {...project} delay={index * 0.06} />)}</div></section>
      ...
      <section id="contact" className="mx-auto max-w-[1500px] px-4 py-24 sm:px-8"><SectionHeader eyebrow="Ready when you are" title="Let’s build something useful." subtitle="A friendly, direct contact section in the same paper-and-ink visual language." /><div className="grid gap-6 md:grid-cols-2"><GlassPanel className="bg-fresh"><div className="flex flex-col gap-7"><div className="flex items-center gap-4"><div className="rounded-full border-2 border-ink bg-white p-4 shadow-[4px_4px_0_#2c2e2a]"><Mail size={26} /></div><div><p className="text-sm font-black uppercase tracking-[0.12em]">Email</p>{portfolioData.contact.emails.map((email) => <EmailDisplay key={email} email={email} className="block text-lg font-black sm:text-xl" />)}</div></div><div className="flex items-center gap-4"><div className="rounded-full border-2 border-ink bg-white p-4 shadow-[4px_4px_0_#2c2e2a]"><MapPin size={26} /></div><div><p className="text-sm font-black uppercase tracking-[0.12em]">Location</p><p className="text-xl font-black">{portfolioData.contact.location}</p></div></div><div className="flex flex-wrap gap-4 pt-2">{[{ icon: Github, href: portfolioData.contact.github },{ icon: Linkedin, href: portfolioData.contact.linkedin },{ icon: Twitter, href: portfolioData.contact.twitter },{ icon: Telegram, href: portfolioData.contact.telegram },{ icon: Instagram, href: portfolioData.contact.instagram },{ icon: Facebook, href: portfolioData.contact.facebook }].map((social, i) => <a key={i} href={social.href} target="_blank" rel="noopener noreferrer" className="rounded-full border-2 border-ink bg-white p-3 shadow-[3px_3px_0_#2c2e2a] transition hover:-translate-y-1"><social.icon size={22} /></a>)}</div></div></GlassPanel><GlassPanel className="bg-white"><form className="space-y-5" action="https://formspree.io/f/xvzbgbrr" method="POST"><div className="grid gap-5 sm:grid-cols-2"><div><label className="mb-2 block text-sm font-black uppercase tracking-[0.12em]">Name</label><input type="text" name="name" className="w-full rounded-full border-2 border-ink bg-cream px-5 py-4 text-base font-bold text-ink outline-none" placeholder="Your Name" required /></div><div><label className="mb-2 block text-sm font-black uppercase tracking-[0.12em]">Email</label><input type="email" name="email" className="w-full rounded-full border-2 border-ink bg-cream px-5 py-4 text-base font-bold text-ink outline-none" placeholder="your@email.com" required /></div></div><div><label className="mb-2 block text-sm font-black uppercase tracking-[0.12em]">Message</label><textarea rows={5} name="message" className="w-full rounded-[2rem] border-2 border-ink bg-cream px-5 py-4 text-base font-bold text-ink outline-none" placeholder="Tell me about your project" required /></div><button className="inline-flex w-full items-center justify-center gap-2 rounded-full border-2 border-ink bg-fresh px-8 py-4 text-lg font-black shadow-[5px_5px_0_#2c2e2a] transition hover:-translate-y-1"><Send size={20} /> Send Message</button></form></GlassPanel></div></section>
